...
_id_では前述のmenuに対応したIDを指定します。
_type_に*html*を指定することで、"_html_"で指定した文字列をそのまま表示します。
...
_id_では前述のmenuに対応したIDを指定します。
_type_に*image*を指定することで、"_src_"で指定したファイルを表示します。
...
output部分では、text, formula, codeのいずれかを指定します。
...
_表示名_には人が識別できる文字列(書籍名や、論文名など)を指定します。
_URL_を指定するとリンクが作成されます。
  * 空文字列を指定すると_表示名_のみ表示され、リンクとなりません。
