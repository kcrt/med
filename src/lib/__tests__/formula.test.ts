...
describe('Formula Evaluation Functions', () => {
  describe('evaluateFormula', () => {
    ...
    it('evaluates expressions with variables', () => {
      expect(evaluateFormula('weight / ((height / 100) ^ 2)', { 
        weight: 70, 
        height: 170 
      })).toBeCloseTo(24.22, 2);
    });
    ...
    it('evaluates iif function', () => {
      expect(evaluateFormula('iif(age >= 18, "adult", "child")', { age: 25 })).toBe('adult');
      expect(evaluateFormula('iif(age >= 18, "adult", "child")', { age: 15 })).toBe('child');
    });
  });
  ...
  describe('evaluateFormulaOutputs', () => {
    ...
    it('evaluates all outputs for calculation formulas', () => {
      ...
      const results = evaluateFormulaOutputs(formula!, { height: 170, weight: 70 });
